...
// manajer & admin_unit (area) disamakan dgn admin/HO utk fungsi shift
// (dibatasi cakupan venue/area di backend)
const canReopen = computed(() => ['admin', 'head_office', 'manager_unit', 'admin_unit'].includes(auth.user?.role))
            ...
            <tbody>
              ...
              <tr v-for="s in shifts" :key="s.id" class="border-t">
                ...
                <td v-if="canDeleteShift || canRincian" class="px-4 py-2 text-right whitespace-nowrap">
                  ...
                  <button v-if="canReopen && s.status === 'open'" @click="openCorrection(s)" class="text-brand-600 text-xs hover:underline ml-3">+ Koreksi</button>
